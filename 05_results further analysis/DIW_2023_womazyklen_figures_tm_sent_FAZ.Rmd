---
title: "Ergebnisse des Topic Models und der Sentiment Analyse für die F.A.Z.-Artikel (1950-2021)"
...
# ------------------------------------------------------------------------------
#
#	 	CONTENT: RESULTS FROM TOPIC MODEL AND SENTIMENT ANALYSIS (F.A.Z.)
#
...
# newspaper articles from Fankfurter Allgemeine Zeitung (F.A.Z.).
# ==============================================================================
```


In dieser Analyse werden alle *F.A.Z.*-Artikel betrachtet, die sich mit dem 
Wohnungsmarkt beschäftigen und zwischen 1950 bis 2021 erschienen sind. Um die 
...
wurden 62829 Artikel identifiziert. Diese wurden mit der Nutzung von Wörterbuch 
...
sOutFile1 = "Draft/figures/sentiment-analysis/Fig_sentiment_analysis_FAZ_sentimerge_sentiments.pdf"
sOutFile2 = "Draft/figures/sentiment-analysis/Fig_sentiment_analysis_FAZ_sentimerge_polarity.pdf"
sOutFile3 = "Draft/figures/topic-modelling/fig_topic_modelling_Zeitverlauf_FAZ.pdf"
sOutFile4 = "Data/FAZ/articlelists_sentiment/Sentiment_FAZ_aggr_subtopics.rds"
```
...
Y = Y %>% mutate(LDA_topic_1_catnew_en = case_when(
  LDA_topic_1==2 |  LDA_topic_1==4 |  LDA_topic_1==7 ~ "market"
  ,LDA_topic_1==5  |  LDA_topic_1==9 |LDA_topic_1==3 ~ "policy"
  ,LDA_topic_1==6 |  LDA_topic_1==10  ~ "other"
  ,LDA_topic_1==1 |  LDA_topic_1==8 ~ "daily life"))
table(Y$LDA_topic_1_catnew_en,useNA="ifany")

# recode sub topics
# ------------------
Y = Y %>% mutate(LDA_topic_1_name_new = case_when(
  LDA_topic_1==1 ~ "Architektur und Stadtbild"
  , LDA_topic_1==2 ~ "Miet- und Baupreisentwicklung"
  , LDA_topic_1==3 ~ "Wohnsituation und -politik (Rhein-Main)"
  , LDA_topic_1==4 ~ "Immobilienmarkt"
  , LDA_topic_1==5 ~ "Wohnungspolitik im Wahlkampf (Rhein-Main)"
  , LDA_topic_1==6 ~ "Weltpolitik"
  , LDA_topic_1==7 ~ "Wohnungsbau (Rhein-Main)"
  , LDA_topic_1==8 ~ "Familienleben und Wohnumfeld"
  , LDA_topic_1==9 ~ "Wohnungspolitik auf Bundesebene"
...
Y_bin <- Y%>% 
  mutate(negative = Sentiment_weight_norm < 0
                     ,positive = Sentiment_weight_norm >= 0)%>%
  group_by(Year)%>%
...
# ----------------------------------------------
Y_bin_lda3 <- Y%>%
  mutate(negative = as.numeric(Sentiment_weight_norm < 0)
         ,positive = as.numeric(Sentiment_weight_norm >= 0)
...
# compute binary sentiment score (by main topic, english version)
# ----------------------------------------------------------------
Y_bin_lda3_en <- Y%>%
  mutate(negative = as.numeric(Sentiment_weight_norm < 0)
         ,positive = as.numeric(Sentiment_weight_norm >= 0)
...
wurden vier Kategorien ermittelt: Politik, Wirtschaft, Wohnraum als Lebensraum, Andere Politikfelder sowie 10 Unterthemen.

Die folgende Tabelle gibt eine Übersicht über die Themen.

|Nr.|Kategorie|Name|
|-|---|----|
|3|Politik|Regionales Wohnungsangebot und -politik (Rhein-Main)|
|5|Politik|Wohnungspolitik im Wahlkampf (Rhein-Main)|	
|9|Politik|Wohnungspolitik auf Bundesebene|
|2|Wirtschaft|Miet- und Baupreisentwicklung|
|4|Wirtschaft|Immobilienmarkt|
|7|Wirtschaft|Wohnungsbau regional (Rhein-Main)|
|8|Wohnraum als Lebensraum|Familienleben und Wohnumfeld|
|1|Wohnraum als Lebensraum|Architektur und Stadtbild|
|6|Andere Politikfelder|Außenpolitik|
|10|Andere Politikfelder|Unternehmen und Arbeitswelt|	
...
ggplot(Y_aggr_lda_main, aes(x = Year))+
    ...
    labs(title = "Anzahl der Wohnungsmarktartikel nach Kategorien"
       ,subtitle = "in der F.A.Z. (1950 - 2021)"
...
plot1a <- ggplot(Y_aggr_lda_sub%>% 
   ...
   labs(title = "Artikel in der Kategorie Wirtschaft"
       ,subtitle = "in der F.A.Z. (1950 - 2021)",y = "Anzahl der Artikel"
       ,x = "Jahr")+
    facet_grid(rows=vars(fct_relevel(LDA_topic_1_name_new,"Miet- und Baupreisentwicklung","Immobilienmarkt","Wohnungsbau (Rhein-Main)"))
...
der emotionalen Ladung betrachtet, d.h. ein Artikel kann nicht mehr positiver als 
ein anderer sein. Damit kann dann die Anzahl posititiv und negativ 
klassifizierter Artikel dargestellt und verglichen werden.

Die folgenden Abbildungen zeigen die Polarität der F.A.Z.-Artikel ohne 
...
plot2c <- ggplot(Y_bin, aes (x= Year, y = Sent_perc_pos))+
  ...
  scale_x_continuous(expand = c(0, 0), breaks=seq(1950,2021,5)) +
...
# ==============================================================================
# 4.2) Polarity by main topics
# ==============================================================================

# plot number of positive and negative articles per year (by main topic)
# ----------------------------------------------------------------------
...
plot3a <- ggplot(Y_bin_lda3, aes(x = Year))+
  ...
  labs(
    ...
    y = "Anzahl der Artikel",
    x = "Jahr")+
  scale_colour_manual(name="",values=c("positiv"="green"
                                       ,"negativ"="red","Differenz"="black")) +
      scale_x_continuous(limits = c(1949,2022), breaks = seq(1950,2021,20))+
    theme_bw() + theme(panel.border = element_blank()
                     ,strip.background = element_blank()
...
plot3b <- ggplot(Y_bin_lda3_en, aes(x = Year))+
  ...
  labs(
    title = "Number of positive and negative articles on housing issues",
    subtitle =  "in the newspaper F.A.Z. (1950 - 2021)",
    y = "number of articles",
    x = "year")+
  scale_colour_manual(name="",values=c("positive"="green"
                                       ,"negative"="red","difference"="black")) +
      scale_x_continuous(limits = c(1949,2022), breaks = seq(1950,2021,20))+
    theme_bw() + theme(panel.border = element_blank()
                     ,strip.background = element_blank()
                     ,strip.text = element_text(size=12)
...
plot3c <- ggplot(Y_bin_lda3, aes(x = Year))+
  ...
  labs(
    title = "Differenz positiver vs. negativer Artikel zum Thema Wohnungsmarkt",
    subtitle =  "in der F.A.Z. (1950 - 2021)",y = "Anzahl der Artikel",x = "Jahr")+
    facet_wrap(~LDA_topic_1_catnew)+
  theme_bw() + theme(panel.border = element_blank()
                     ,panel.grid.major = element_blank()
                     ,panel.grid.minor = element_blank()
...
plot3d <- ggplot(Y_bin_lda3, aes (x= Year, y = Sent_perc_pos))+
  ...
  geom_hline(yintercept = 50, colour = "light blue", size = 1, linetype = "dashed")+
  scale_color_manual(values = c("Politik" = "darkslateblue"
                               ,"Wirtschaft" = "darkgoldenrod"
                               ,"Wohnraum als Lebensraum" = "darkgreen"))+
  scale_y_continuous(expand = c(0, 0), limits=c(0,100))+
  scale_x_continuous(expand = c(0, 0), breaks=seq(1950,2021,5)) +
  labs(title = "Anteil positiver Artikel zum Thema Wohnungsmarkt nach Kategorien"
       ,subtitle = "in der F.A.Z. (1950 - 2021)"
...
ggplot(Y_aggr_lda_sub%>% 
    ...
    geom_col()+
    labs(title = "Artikel in der Kategorie Wirtschaft"
       ,subtitle = "in der F.A.Z. (1950 - 2021)",y = "Sentimentwert",x = "Jahr")+
    facet_grid(rows=vars(LDA_topic_1_name_new)
               ,labeller=label_wrap_gen(width = 30, multi_line = TRUE))+
  theme_classic()+
  theme(strip.text.y.right = element_text(angle = 0))
...
plot6a <- ggplot(Y_aggr_lda_sub%>% 
  ...
  geom_smooth(aes(color = LDA_topic_1_name_new), size = 0.8, span = 0.2, se = F
              , method="loess",formula=y~x)+
    scale_color_manual(values=c("darkgoldenrod4","darkorange","darkgoldenrod1"))+
  geom_hline(yintercept = 0, colour = "light blue", size = 1, linetype = "dashed")+
  labs(title = "Durchschnitt Sentimentwerte der Artikel"
       ,subtitle = "in der F.A.Z. (1950 - 2021) | Kategorie Wirtschaft"
...
plot6b <- ggplot(Y_aggr_lda_sub%>% 
  ...
  geom_smooth(aes(color = LDA_topic_1_name_new), size = 0.8, span = 0.2, se = F
              , method="loess",formula=y~x)+
      scale_color_manual(values=c("darkslateblue","deeppink4","darkorchid1","darksalmon"))+
  geom_hline(yintercept = 0, colour = "light blue", size = 1, linetype = "dashed")+
  labs(title = "Durchschnitt Sentimentwerte der Artikel"
       ,subtitle = "in der F.A.Z. (1950 - 2021) | Kategorie Politik"
       ,y = "Artikeldurchschnitt Sentiment"
       ,x = "Jahr"
       ,color = "")+
    theme_bw() + theme(panel.border = element_blank()
...
plot6c <- ggplot(Y_aggr_lda_sub%>% 
  ...
  geom_hline(yintercept = 0, colour = "light blue", size = 1, linetype = "dashed")+
  labs(title = "Durchschnitt Sentimentwerte der Artikel"
       ,subtitle = "in der F.A.Z. (1950 - 2021) | Kategorie Wohnraum als Lebensraum"
       ,y = "Artikeldurchschnitt Sentiment"
       ,x = "Jahr"
       ,color = "")+
    theme_bw() + theme(panel.border = element_blank()
